function [history,res] = admm_tv_sparse_wo_inverse_nonnegative(A,b,rho,mu,lambda,tau,step_length,MAX_ITER,varargin)
...
for i = 1:MAX_ITER
 %% x-update
 x = x - delta*(y1 + D'*y2 + y3)+delta*rho*(x-w)+delta*rho*D'*(D*x-z)+delta*mu*A'*(b-A*x) + delta*rho*(x-s);
 %% w-update
 w_old = w;
 e = x + y1/rho;
